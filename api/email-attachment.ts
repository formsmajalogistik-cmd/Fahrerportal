// GET /api/email-attachment?mailbox=...&messageId=...&attachmentId=...[&disposition=inline|attachment]
// Lädt einen Anhang als Binär-Stream — gleicher Pattern wie /api/download.

import { getAuthedUser, HttpError } from '../server-lib/auth.js';
import { assertMailboxAllowed } from '../server-lib/mailboxAuth.js';
import { getAttachmentBytes } from '../server-lib/graph.js';
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    console.error('[/api/email-attachment]', err);
