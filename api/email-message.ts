// GET /api/email-message?mailbox=...&id=...
// Lädt eine einzelne Nachricht inkl. Body + Anhangs-Metadaten.

import { getAuthedUser, HttpError } from '../server-lib/auth.js';
import { assertMailboxAllowed } from '../server-lib/mailboxAuth.js';
import { getMessage } from '../server-lib/graph.js';
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    console.error('[/api/email-message]', err);
